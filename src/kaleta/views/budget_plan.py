...
def register() -> None:
    ...
    async def budget_plan_page() -> None:
        ...
        with page_layout(t("budget_plan.title")):
            ...
            # ── Copy-from-previous-month control ────────────────────────────
            month_opts: dict[int, str] = {m: _month_labels()[m - 1] for m in range(1, 13)}
            default_target = max(today.month, 2)  # Jan has no "previous" in-year target
            ...
            async def _copy_from_prev() -> None:
                ...
                async with AsyncSessionFactory() as session:
                    written = await BudgetService(session).copy_forward(
                        from_y, from_m, target_y, target_m
                    )
                if written == 0:
