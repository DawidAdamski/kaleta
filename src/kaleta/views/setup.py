...
def _make_picker(
    ...
    """
    state: dict = {"cwd": Path.home()}

    with ui.dialog() as dialog, ui.card().classes("w-full max-w-xl p-0 overflow-hidden"):
        # Header: current path
        cwd_label = ui.label("").classes(
            "text-xs font-mono px-4 py-2 bg-grey-2 w-full truncate"
        )
...
def _render_recent() -> None:
    ...
    recent = get_recent()
    if not recent:
        ui.label(t("setup.no_recent")).classes(
            "text-sm text-grey-5 py-6 text-center w-full"
        )
        return
    ...
    for entry in recent:
        ...
        async def _handle_pick(e=entry) -> None:
            spinner = ui.spinner(size="sm")
            status = ui.label("").classes("text-sm text-grey-6")
            await _activate_db(
                e["url"], name=e.get("name", ""), spinner=spinner, status=status
            )
...
def register() -> None:  # noqa: PLR0915
    ...
    async def setup_page() -> None:  # noqa: PLR0915
        ...
        with ui.column().classes(
            ...
            ui.label(t("setup.welcome")).classes(
                "text-lg text-grey-6 text-center max-w-lg"
            )

            # ── Step 1: Local vs Cloud ──
            with ui.column().classes("w-full max-w-2xl gap-4") as step_choose:
                ui.label(t("setup.choose_storage")).classes(
                    "text-xl font-semibold text-center"
                )

                with ui.row().classes("w-full gap-4 justify-center"):
                    with ui.card().classes(  # noqa: SIM117
                        "flex-1 cursor-pointer hover:shadow-lg transition-shadow p-6"
                    ...
                    with ui.card().classes("flex-1 p-6 opacity-60"):  # noqa: SIM117
                        with ui.column().classes("items-center gap-3 text-center"):
                        ...
                        with ui.card().classes("w-full p-6"):
                            with ui.column().classes("w-full gap-4"):
                                ...
                                with ui.row().classes("w-full items-center gap-1"):
                                    ...
                                    _, open_folder_picker = _make_picker(
                                        new_folder, mode="folder"
                                    )
                                    ui.button(
                                        icon="folder_open",
                            ...
                            with ui.column().classes("w-full gap-4"):
                                ...
                                with ui.row().classes("w-full items-center gap-1"):
                                    open_path = ui.input(
                                        t("setup.db_path"),
                                        placeholder=str(
                                            Path.home() / "Documents" / "budget.db"
                                        ),
                                    ).classes("flex-1")
                                    _, open_file_picker = _make_picker(
                                        open_path, mode="file"
                                    )
                                    ui.button(
                                        icon="folder_open",
                                ...
                                async def _handle_open() -> None:
                                    ...
                                    if not path:
                                        ui.notify(
                                            t("setup.error_path_empty"), type="warning"
                                        )
                                        return
