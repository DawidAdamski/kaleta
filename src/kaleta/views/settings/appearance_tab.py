...
The sidebar card went with artboard `1e`: the top bar replaced the docked
drawer, so there is no expanded-or-collapsed left to choose between.
"""
...
def render_appearance_tab() -> None:
    with (
        ui.row().classes("w-full gap-6 flex-wrap items-start"),
        ui.card().classes("p-6 min-w-72 w-80"),
    ):
